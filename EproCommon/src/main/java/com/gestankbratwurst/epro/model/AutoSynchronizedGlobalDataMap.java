...
public class AutoSynchronizedGlobalDataMap<K, V> extends GlobalDataMap<K, V> {
  ...
  private void fireUpdate(K key) {
    ...
    SerializedUnit<K> serializedUnit = new SerializedUnit<>(contextKey.keyClass, json);
    AutoSyncUpdateEvent<K, V> event = new AutoSyncUpdateEvent<>(contextKey, serializedUnit);
    EproGateway.getRemoteEventManager().callEvent("_AUTO_SYNC_", event);
  }
